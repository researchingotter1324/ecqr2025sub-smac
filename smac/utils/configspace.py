...
from ConfigSpace.util import (
    ...
    get_one_exchange_neighbourhood as _get_one_exchange_neighbourhood,
)
...
def get_one_exchange_neighbourhood(configuration, seed=None, stdev=0.05, num_neighbors=8):
    """Wrapper for ConfigSpace get_one_exchange_neighbourhood that handles numpy type conversion."""
    # Convert any numpy types to native Python types in the configuration and parameters
    from ConfigSpace import Configuration
    import numpy as np
    
    # Convert seed to native Python int if it's a numpy type
    if seed is not None:
    ...
    # Convert stdev and num_neighbors if they're numpy types
    if hasattr(stdev, 'item'):
        stdev = float(stdev.item())
    elif isinstance(stdev, np.floating):
        stdev = float(stdev)
        
    if hasattr(num_neighbors, 'item'):
        num_neighbors = int(num_neighbors.item())
    elif isinstance(num_neighbors, np.integer):
    ...
    if isinstance(configuration, Configuration):
        ...
        # Try to create a new configuration with converted types
        try:
            converted_config = Configuration(configuration.configuration_space, values=converted_dict)
            return _get_one_exchange_neighbourhood(converted_config, seed=seed, stdev=stdev, num_neighbors=num_neighbors)
        except Exception:
            # If conversion fails, try with original configuration
            return _get_one_exchange_neighbourhood(configuration, seed=seed, stdev=stdev, num_neighbors=num_neighbors)
    else:
        return _get_one_exchange_neighbourhood(configuration, seed=seed, stdev=stdev, num_neighbors=num_neighbors)
...
def transform_continuous_designs(
    ...
    for vector in design:
        try:
            from ConfigSpace import Configuration
            conf = Configuration(configspace, vector=vector)
