...
class LocalSearch(AbstractAcquisitionMaximizer):
    ...
    def _safe_get_neighborhood(self, configuration, seed=None, stdev=0.05, num_neighbors=8):
        """Safe wrapper for get_one_exchange_neighbourhood that handles numpy type issues."""
        try:
            return get_one_exchange_neighbourhood(configuration, seed=seed, stdev=stdev, num_neighbors=num_neighbors)
        except TypeError as e:
            if "expected int, got numpy.float64" in str(e):
                # Fallback: return an empty iterator if numpy types cause issues
                logger = get_logger(self.__class__.__module__ + "." + self.__class__.__name__)
                logger.warning(f"ConfigSpace neighborhood generation failed due to numpy type issue: {e}. Using empty neighborhood.")
                return iter([])
            else:
                raise e
    ...
    def _search(
        ...
        for i, inc in enumerate(candidates):
            neighborhood_iterators.append(
                ...
                #     Optimization(LION 5)
                self._safe_get_neighborhood(inc, seed=self._rng.randint(low=0, high=100000))
            )
        ...
        while np.any(active):
                ...
                if active[i]:
                    ...
                    for j in range(obtain_n[i]):
                        try:
                            ...
                            # an invalid configuration, or `TypeError` for numpy type issues.
                            logger.debug(e)
                            if "expected int, got numpy.float64" in str(e):
                                logger.warning(f"Skipping neighborhood generation due to numpy type issue: {e}")
                            new_neighborhood[i] = True
                            break  # Stop trying to get more neighbors from this iterator
            ...
            for i in range(num_candidates):
                ...
                if new_neighborhood[i]:
                    ...
                    neighborhood_iterators[i] = self._safe_get_neighborhood(
                        candidates[i],
