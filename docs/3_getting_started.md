...
myfloat = UniformFloatHyperparameter("myfloat", 0.1, 1.5)          # Uniform Float
myint = UniformIntegerHyperparameter("myint", 2, 10)               # Uniform Integer
species = CategoricalHyperparameter("species", ["mouse", "cat", "dog"])  # Categorical
cs.add_hyperparameters([myfloat, myint, species])
```
