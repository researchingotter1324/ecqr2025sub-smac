...
This is a single commit reuploaded copy of a fork of SMAC that resolves some minor versioning conflicts.

The original README by SMAC is reported below, and the original fork will be referenced instead of this single commit version in the final paper, if accepted.
...
C = UniformFloatHyperparameter("C", 0.100, 1000.0)
configspace.add_hyperparameters([C])
